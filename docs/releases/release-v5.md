...
- **`@HOME@` portability.** Configs use a `@HOME@` sentinel the installer rewrites to the real home directory at deploy time (legacy mirrors' `@HOME@` is also rewritten), so it works for any username.
