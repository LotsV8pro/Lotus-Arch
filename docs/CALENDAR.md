...
`sync-calendars.py` syncs **every** Google Calendar on your account to `~/.config/inir-birthdays/`, one `.ics` file per calendar. The primary calendar (holds your normal agenda) is one source; the **"Cumpleaños (Contactos)"** calendar — generated by Google from your Contacts' birthday fields — is another. The shell's config wires up three sources by default:

| Config id | Contents | File |
|---|---|---|
| `cal_00_sync` | your primary Google Calendar (agenda + any manual events) | `00.ics` |
| `cal_01_sync` | Spain public holidays | `01.ics` |
| `cal_03_sync` | **birthdays from Google Contacts** ("Cumpleaños") | `02.ics` |

The files are named **portably** (`NN.ics`) so no personal data — like the email address a primary Google Calendar is titled with — ever ends up on disk or in the repo.
