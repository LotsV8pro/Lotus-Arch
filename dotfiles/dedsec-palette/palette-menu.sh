...
save_preset() {
    local name
    name=$(rofi -dmenu -p "  Preset Name" -config "$MENU_THEME" 2>/dev/null)
...
case "$chosen" in
    *Presets)
        ...
        preset_choice=$(printf '%s\n' "${preset_list[@]}" \
            | rofi -dmenu -p "  Presets" -config "$MENU_THEME" -no-custom 2>/dev/null)
    ...
    *Edit*)
        ...
        chosen_color=$(printf '%s\n' "${menu_entries[@]}" | rofi -dmenu -p "  Edit Color" -config "$MENU_THEME" -no-custom 2>/dev/null)
        ...
        input_method=$(printf '%s\n' \
            "  Visual RGB Picker" \
            "  Hex Input" \
            | rofi -dmenu -p "  ${labels[$chosen_key]}" -config "$MENU_THEME" -no-custom 2>/dev/null)
        ...
        if [[ "$input_method" == *"Visual"* ]]; then
            ...
            if [[ "$new_value" == "HEX_INPUT" ]]; then
                new_value=$(echo "$current_value" | rofi -dmenu -p "  Hex Value" -config "$MENU_THEME" 2>/dev/null)
            fi
        else
            new_value=$(echo "$current_value" | rofi -dmenu -p "  Hex Value" -config "$MENU_THEME" 2>/dev/null)
        fi
