...
# define the config files (check both .lua and .conf since both may be active)
...
for f in \
  ...
  "$HOME/.config/hypr/configs/Keybinds.lua" \
  "$HOME/.config/hypr/UserConfigs/UserKeybinds.conf" \
  "$HOME/.config/hypr/UserConfigs/UserKeybinds.lua" \
  "$HOME/.config/hypr/UserConfigs/Laptops.conf" \
  "$HOME/.config/hypr/UserConfigs/Laptops.lua"; do
  [[ -f "$f" ]] && files+=("$f")
done
