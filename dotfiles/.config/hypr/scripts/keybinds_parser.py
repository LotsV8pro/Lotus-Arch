#!/usr/bin/env python3
import sys
import re
import os

def normalize_combo(combo):
    return combo.replace(" ", "").replace("\t", "")

def extract_combo(line):
    # Remove comments and whitespace
    line = re.sub(r'\s*#.*$', '', line).strip()
    
    if '=' not in line:
        return None
        
    try:
        rhs = line.split('=', 1)[1]
        parts = [p.strip() for p in rhs.split(',')]
        if len(parts) < 2:
            return None
            
        mods = parts[0]
        key = parts[1]
        return f"{mods},{key}"
    except Exception:
        return None

def parse_lua_files(files):
    """Parse .lua format: hl.bind(key, action, opts) and hl.unbind(key)"""
    import ast
    
    binding_map = {}
    source_map = {}
    user_bind_map = {}
    unbound_user = {}
    seen_any_bind = {}
    default_seen = {}
    
    if not files:
        return [], []
    
    user_conf_path = files[-1] if len(files) > 1 else None
    
    for file_path in files:
        if not os.path.exists(file_path):
            continue
        
        try:
            with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                content = f.read()
            
            # Match hl.bind(...) and hl.unbind(...) calls
            # Handle multiline by joining
            is_user_file = (file_path == user_conf_path)
            
            # Simple regex-based extraction for hl.bind and hl.unbind
            # Pattern: hl.bind("KEY", ..., { description = "DESC" })
            # Pattern: hl.unbind("KEY")
            
            bind_pattern = re.compile(r'hl\.bind\s*\(\s*"([^"]+)"', re.MULTILINE)
            unbind_pattern = re.compile(r'hl\.unbind\s*\(\s*"([^"]+)"', re.MULTILINE)
            
            # Extract descriptions
            desc_pattern = re.compile(r'description\s*=\s*"([^"]+)"', re.MULTILINE)
            
            lines = content.split('\n')
            for i, line in enumerate(lines):
                line = line.strip()
                
                # Check for hl.unbind
                um = unbind_pattern.search(line)
                if um:
                    key_combo = um.group(1)
                    # Normalize: replace " + " with "+", uppercase modifiers
                    key_combo = key_combo.replace(" + ", "+").replace(" +", "+").replace("+ ", "+")
                    key_norm = normalize_combo(key_combo)
                    
                    if is_user_file:
                        unbound_user[key_norm] = True
                    
                    if key_norm in binding_map:
                        del binding_map[key_norm]
                    if key_norm in source_map:
                        del source_map[key_norm]
                    continue
                
                # Check for hl.bind
                bm = bind_pattern.search(line)
                if bm:
                    key_combo = bm.group(1)
                    # Normalize key combo
                    key_combo = key_combo.replace(" + ", "+").replace(" +", "+").replace("+ ", "+")
                    key_norm = normalize_combo(key_combo)
                    
                    seen_any_bind[key_norm] = True
                    
                    if not is_user_file:
                        default_seen[key_norm] = True
                    
                    # Try to find description in nearby lines (within 3 lines)
                    desc = ""
                    for j in range(i, min(i+4, len(lines))):
                        dm = desc_pattern.search(lines[j])
                        if dm:
                            desc = dm.group(1)
                            break
                    
                    # Store as pseudo-conf line for format_for_rofi
                    pseudo_line = f'bindd = {key_combo.replace("+", ", ")}, {desc}, exec, (lua bind)'
                    
                    if key_norm not in source_map:
                        binding_map[key_norm] = pseudo_line
                        source_map[key_norm] = file_path
                    
                    if is_user_file:
                        user_bind_map[key_norm] = pseudo_line
                        binding_map[key_norm] = pseudo_line
                        source_map[key_norm] = file_path
        
        except Exception as e:
            sys.stderr.write(f"Error reading {file_path}: {e}\n")
            continue
    
    raw_keybinds = []
    missing_unbind_suggestions = []
    
    for combo in seen_any_bind:
        eff_line = binding_map.get(combo)
        src = source_map.get(combo)
        
        if not eff_line:
            continue
        
        raw_keybinds.append(eff_line)
        
        if (src == user_conf_path and 
            combo in default_seen and 
            combo not in unbound_user):
            missing_unbind_suggestions.append(f"unbind {combo}")
    
    return raw_keybinds, missing_unbind_suggestions


def format_lua_keybinds(raw_binds):
    """Format lua keybinds for display in rofi"""
    formatted_lines = []
    
    for line in raw_binds:
        # line is like "bindd = SUPER, D, App launcher, exec, (lua bind)"
        match = re.match(r'^\s*bindd\s*=\s*(.*)', line)
        if not match:
            continue
        
        rhs = match.group(1).strip()
        parts = [p.strip() for p in rhs.split(',')]
        
        if len(parts) < 2:
            continue
        
        mods = parts[0]
        key = parts[1]
        desc = parts[2] if len(parts) >= 3 else ""
        
        # Build combo string
        mods = re.sub(r'[ \t]+', '+', mods)
        
        if mods and key:
            combo_str = f"{mods}+{key}"
        elif key:
            combo_str = key
        else:
            combo_str = mods
        
        if desc and desc != "(lua bind)":
            formatted_lines.append(f"{combo_str} — {desc}")
        else:
            formatted_lines.append(combo_str)
    
    return formatted_lines


def parse_conf_files(files):
    """Parse .conf format (original hyprlang syntax)"""
    binding_map = {}
    source_map = {}
    user_bind_map = {}
    unbound_user = {}
    seen_any_bind = {}
    default_seen = {}
    
    if not files:
        return [], []
    
    user_conf_path = files[-1] if len(files) > 1 else None

    for file_path in files:
        if not os.path.exists(file_path):
            continue
        
        try:
            with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                for line in f:
                    line = line.rstrip('\n')
                    if not line or line.strip().startswith('#'):
                        continue
                    
                    is_bind = re.match(r'^\s*bind[a-z]*\s*=', line)
                    is_unbind = re.match(r'^\s*unbind\s*=', line)
                    
                    if is_bind:
                        combo_raw = extract_combo(line)
                        if not combo_raw:
                            continue
                        combo = normalize_combo(combo_raw)
                        seen_any_bind[combo] = True
                        
                        is_user_file = (file_path == user_conf_path)
                        
                        if not is_user_file:
                            default_seen[combo] = True
                        
                        if combo not in source_map:
                            binding_map[combo] = line
                            source_map[combo] = file_path
                        
                        if is_user_file:
                            user_bind_map[combo] = line
                            binding_map[combo] = line
                            source_map[combo] = file_path
                    
                    elif is_unbind:
                        combo_raw = extract_combo(line)
                        if not combo_raw:
                            continue
                        combo = normalize_combo(combo_raw)
                        
                        if file_path == user_conf_path:
                            unbound_user[combo] = True
                        
                        if combo in binding_map:
                            del binding_map[combo]
                        if combo in source_map:
                            del source_map[combo]
        
        except Exception as e:
            sys.stderr.write(f"Error reading {file_path}: {e}\n")
            continue

    raw_keybinds = []
    missing_unbind_suggestions = []
    
    for combo in seen_any_bind:
        eff_line = binding_map.get(combo)
        src = source_map.get(combo)
        
        if not eff_line:
            continue
        
        raw_keybinds.append(eff_line)
        
        if (src == user_conf_path and 
            combo in default_seen and 
            combo not in unbound_user):
            suggest = re.sub(r'^\s*bind[a-z]*', 'unbind', eff_line)
            missing_unbind_suggestions.append(suggest)
    
    return raw_keybinds, missing_unbind_suggestions

def parse_files(files):
    """Detect file format and parse accordingly"""
    has_lua = any(f.endswith('.lua') for f in files if os.path.exists(f))
    has_conf = any(f.endswith('.conf') for f in files if os.path.exists(f))
    
    if has_lua and not has_conf:
        return parse_lua_files(files)
    elif has_lua and has_conf:
        lua_files = [f for f in files if f.endswith('.lua')]
        conf_files = [f for f in files if f.endswith('.conf')]
        lua_binds, lua_sugg = parse_lua_files(lua_files)
        conf_binds, conf_sugg = parse_conf_files(conf_files)
        return lua_binds + conf_binds, lua_sugg + conf_sugg
    else:
        return parse_conf_files(files)


def format_for_rofi(raw_binds):
    formatted_lines = []
    
    for line in raw_binds:
        # Check if this is a lua pseudo-line
        if line.startswith('bindd = ') and '(lua bind)' in line:
            # Already formatted from parse_lua_files
            match = re.match(r'^\s*bindd\s*=\s*(.*)', line)
            if not match:
                continue
            rhs = match.group(1).strip()
            parts = [p.strip() for p in rhs.split(',')]
            if len(parts) < 2:
                continue
            mods = parts[0]
            key = parts[1]
            desc = parts[2] if len(parts) >= 3 else ""
            mods = re.sub(r'[ \t]+', '+', mods)
            combo_str = f"{mods}+{key}" if mods and key else (key or mods)
            if desc and desc != "(lua bind)":
                formatted_lines.append(f"{combo_str} — {desc}")
            else:
                formatted_lines.append(combo_str)
            continue
        
        # Original .conf format parsing
        match = re.match(r'^\s*(bind[a-z]*)\s*=(.*)', line)
        if not match:
            continue
        
        binder = match.group(1).replace(" ", "").replace("\t", "")
        rhs = match.group(2).strip()
        
        has_desc = 'd' in binder and binder != 'bind'
        
        parts = [p.strip() for p in rhs.split(',')]
        
        if len(parts) < 2:
            continue
        
        mods = parts[0]
        key = parts[1]
        
        desc = ""
        dispatcher = ""
        params = ""
        
        start_idx = 0
        
        if has_desc:
            desc = parts[2] if len(parts) >= 3 else ""
            dispatcher = parts[3] if len(parts) >= 4 else ""
            start_idx = 4
        else:
            dispatcher = parts[2] if len(parts) >= 3 else ""
            start_idx = 3
        
        remaining_parts = []
        if start_idx < len(parts):
            for i in range(start_idx, len(parts)):
                if parts[i]:
                    remaining_parts.append(parts[i])
        
        if remaining_parts:
            params = ", ".join(remaining_parts)
        
        mods = mods.replace("$mainMod", "SUPER")
        mods = re.sub(r'[ \t]+', '+', mods)
        
        if mods and key:
            combo_str = f"{mods}+{key}"
        elif key:
            combo_str = key
        else:
            combo_str = mods
        
        if has_desc and desc:
            formatted_lines.append(f"{combo_str} — {desc}")
        elif dispatcher:
            if params:
                formatted_lines.append(f"{combo_str} — {dispatcher} {params}")
            else:
                formatted_lines.append(f"{combo_str} — {dispatcher}")
        else:
            formatted_lines.append(combo_str)
    
    return formatted_lines

def main():
    if len(sys.argv) < 2:
        # No files provided
        sys.exit(0)
        
    config_files = sys.argv[1:]
    
    binds, suggestions = parse_files(config_files)
    
    if not binds:
        print("no keybinds found.")
        sys.exit(1)
        
    formatted = format_for_rofi(binds)
    
    for line in formatted:
        print(line)
        
    # Handle suggestions (print to stderr or a specific file if needed, 
    # but the original script assigns it to a variable 'msg'.
    # To pass this back to bash, we might need a separate mechanism or just print to a known file.)
    if suggestions:
        import tempfile
        try:
            with tempfile.NamedTemporaryFile(mode='w', delete=False, prefix='hypr-unbind-suggestions-', suffix='.conf') as tf:
                tf.write('\n'.join(suggestions) + '\n')
                # We print a special marker line to stdout that the bash script can capture?
                # Or better, just print to stderr and let the user ignore it, 
                # OR, since the original script specifically puts it in the Rofi message,
                # we can print a special string at the END of stdout or to a side channel.
                
                # Let's decide to print the valid keybinds to stdout (for rofi).
                # And print the suggestion file path to a known location or specific fd if possible.
                # Simplest: Write to a fixed temp file location that the bash script checks.
                with open("/tmp/hypr_keybind_suggestions_file", "w") as sf:
                    sf.write(tf.name)
        except Exception:
            pass

if __name__ == "__main__":
    main()
