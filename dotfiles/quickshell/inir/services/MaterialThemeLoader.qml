...
`
    ...
    // ── waybar/external palette export ──────────────────────────────────────
    // waybar, kitty, swaync, hyprlock, etc. read @define-color tokens from
    // ~/.local/state/haku_theme/colors.css. Export the ACTIVE Material palette
    // (the same wallpaper-derived palette the shell panels and desktop widgets
    // use) into that file, so every consumer follows the wallpaper palette
    // instead of a separate ColorThief accent.
    property string hakuAccentPath: `${Directories.stateUserPath}/generated/haku_palette.css`
    property string _lastWrittenCss: ""

    function _waybarPaletteCss(): string {
        const ov = root._styleAppPaletteOverlay()
        if (!ov) return ""
        const m3 = Appearance.m3colors
        const err = m3.m3error !== undefined ? root.colorToHex(m3.m3error) : "#B3261E"
        return "/* Generated by MaterialThemeLoader from the active palette */\n"
            + "@define-color accent_color " + ov.primary + ";\n"
            + "@define-color on_accent " + ov.on_primary + ";\n"
            + "@define-color accent_container " + ov.primary_container + ";\n"
            + "@define-color on_accent_container " + ov.on_primary_container + ";\n"
            + "@define-color background " + ov.background + ";\n"
            + "@define-color on_background " + ov.on_background + ";\n"
            + "@define-color surface " + ov.surface_container_low + ";\n"
            + "@define-color surface_container " + ov.surface_container + ";\n"
            + "@define-color surface_high " + ov.surface_container_high + ";\n"
            + "@define-color on_surface " + ov.on_surface + ";\n"
            + "@define-color on_surface_variant " + ov.outline_variant + ";\n"
            + "@define-color outline " + ov.outline + ";\n"
            + "@define-color error " + err + ";\n"
    }

    function _exportWaybarPalette(): void {
        if (!root.defaultApplyExternal) return
        const css = root._waybarPaletteCss()
        if (!css || css === root._lastWrittenCss) return
        root._lastWrittenCss = css
        root._log("[MaterialThemeLoader] Exporting palette css:", css.replace(/\n/g, " | "))
        // Write inside the quickshell state dir (exec sandbox only allows that
        // and /tmp). ~/.local/state/haku_theme/colors.css is a symlink to this
        // file, so waybar/swaync/kitty follow the wallpaper palette.
        const script = `cat > ${_shq(root.hakuAccentPath)} <<'MTL_EOF'
${css}MTL_EOF
`
        Quickshell.execDetached(["/usr/bin/bash", "-c", script])
    }
