...
Singleton {
    ...
    component BrightnessMonitor: QtObject {
        ...
        onMultipliedBrightnessChanged: {
            ...
            if (monitor.isDdc) {
                // DDC writes are slow (~1-2s) and the panel drops writes that
                // land too close together. Accumulate like brightness-scroll.sh:
                // reset the settle window on every change and write only once
                // the value stops moving (drag released). Avoids overlapping
                // ddcutil setvcp processes during a drag.
                setTimer.restart()
            } else if (!setTimer.running) {
                setTimer.start()
            }
        }
        ...
        readonly property Process initProc: Process {
            ...
            onExited: {
                ...
                if (Number.isFinite(value)) {
                    ...
                    monitor.brightness = value
                    monitor.ready = true
                    // Do NOT sync() here: an init read failure must not clobber
                    // the hardware value that waybar/scripts set while the shell
                    // was asleep or restarting. Restores after real sleeps go
                    // through restoreAfterWake(). Report the fallback instead.
                    console.warn(`[Brightness] ${screenName}: init read failed, ` +
                        `showing last known ${value.toFixed(2)} without writing`)
                    return
                }
                monitor.ready = true
            }
