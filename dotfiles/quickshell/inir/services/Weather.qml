...
Singleton {
    ...
    // Location mode: "auto" (IP/GPS) or "manual" (city/coords)
    readonly property string locationMode: Config.options?.bar?.weather?.mode ?? "auto"
    readonly property bool manualMode: root.locationMode === "manual"
    ...
    // Resolve location: manual mode uses city/coords, auto mode uses GPS > IP
    function resolveLocation(): void {
        ...
        if (!root.manualMode) {
            // Auto: GPS if enabled, else IP auto-detect
            if (root.enableGPS) {
        ...
        // Manual mode but no city/coords configured — fall back to IP
        console.info("[Weather] Manual mode without location set, falling back to IP...");
        root.getLocation();
    }
