...
Singleton {
    ...
    // Locale params for Google News.
    // For "local" geo feeds, always use es/ES since geo detection
    // only works with the country's own locale.
    readonly property string _systemLang: Qt.locale().name.split("_")[0] || "en"
    readonly property string _systemCountry: (Qt.locale().name.split("_")[1] || "US")

    function _localeQuery(mode) {
        const lang = (mode === "local") ? "es" : root._systemLang
        const country = (mode === "local") ? "ES" : root._systemCountry
        return `hl=${lang}&gl=${country}&ceid=${country}:${lang}`
    }
    ...
    function feedUrl(mode, topic) {
        const lq = root._localeQuery(mode)
        if (mode === "local" && root.localCity.length > 0)
            return `https://news.google.com/rss/headlines/section/geo/${encodeURIComponent(root.localCity)}?${lq}`
        if (mode === "topic" && root.topics.includes(topic))
            return `https://news.google.com/rss/headlines/section/topic/${topic}?${lq}`
        return `https://news.google.com/rss?${lq}`
    }
