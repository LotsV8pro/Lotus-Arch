...
function parseICS(icsText, sourceId, sourceName, sourceColor) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (line === "END:VEVENT" && inEvent) {
            inEvent = false
            // Skip instance overrides (RECURRENCE-ID). They duplicate the
            // generated instance from the recurring master VEVENT.
...
function _expandRecurrence(baseEvent, rrule) {
    ...
    // WEEKLY + BYDAY: FREQ=WEEKLY;BYDAY=MO,TU,WE,TH,FR means the event recurs
    // on EVERY listed weekday (not just the DTSTART weekday every 7 days).
    // Walk day-by-day from DTSTART so each matching weekday gets an instance.
    if (freq === "weekly") {
        const dayMap = { SU: 0, MO: 1, TU: 2, WE: 3, TH: 4, FR: 5, SA: 6 }
        const bydayMatch = rrule.match(/BYDAY=([A-Z]{2}(?:,[A-Z]{2})*)/)
        if (bydayMatch) {
            const days = bydayMatch[1].split(",").map(d => dayMap[d]).filter(d => d !== undefined)
            if (days.length > 0) {
                const current = new Date(startDate)
                current.setDate(current.getDate() + 1) // base event already counted
                let count = 1 // -1: base event is already counted
