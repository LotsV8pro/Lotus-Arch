...
Singleton {
    ...
    // Reconcile night light state once at startup: a stale wlsunset from a
    // previous session survives shell reloads (it runs in its own transient
    // unit), so unless the config enables it, disable() must run on boot.
