...
Singleton {
    ...
    function resolveControllableSink(node) {
        ...
        if (!isEasyEffectsSink)
            return node

        // EasyEffects wires its sink through an internal chain (equalizer ->
        // compressor -> limiter -> ... -> physical device). Walk the link graph
        // from this node to find the first real (non-virtual, non-stream) sink.
        const physical = root.findPhysicalSinkFrom(node)
        if (physical)
            return physical
    ...
    // BFS through PipeWire links from a virtual sink to the first real sink.
    // EasyEffects and similar filters create intermediate nodes, so following
    // links only one hop is not enough. Prefer the node's audio/description over
    // node.driver-id, which can point at the wrong (capture) device.
    function findPhysicalSinkFrom(node): var {
        const startId = Number(node?.id ?? 0)
        ...
        while (queue.length > 0) {
            ...
            for (const link of Pipewire.links.values) {
                const src = link.source
                const tgt = link.target
                if (!src || !tgt) continue
                if (Number(src.id ?? 0) !== next) continue
                const tgtId = Number(tgt.id ?? 0)
                if (visited.has(tgtId)) continue
                visited.add(tgtId)
                if (root.correctType(tgt, true) && !tgt.isStream) {
                    const props = tgt.properties ?? {}
                    const isVirtual = String(props["node.virtual"] ?? "false") === "true"
                    if (!isVirtual)
                        return tgt
                    queue.push(tgtId)
    ...
    // wpctl target: prefer the resolved physical sink ID, fall back to @DEFAULT_AUDIO_SINK@.
    function _sinkWpctlTarget(): string {
    ...
    Timer {
        ...
        onTriggered: {
            ...
            wpctlSetSinkVolume.command = ["wpctl", "set-volume", root._sinkWpctlTarget(), String(root._queuedSinkVolume)]
    ...
    Process {
        ...
        command: ["wpctl", "set-volume", root._sinkWpctlTarget(), "1.0"]
    ...
    Process {
        ...
        command: ["wpctl", "set-volume", root._sinkWpctlTarget(), "2%+"]
    }
