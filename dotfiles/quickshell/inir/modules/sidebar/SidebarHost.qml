...
Scope {
    ...
    readonly property string animationType: Config.options?.sidebar?.animationType ?? "iris"
    ...
    property bool _irisActive: root.animationType === "iris"
        || root._irisOpen || root._irisProgress > 0.0
        || irisProgressAnim.running
    ...
    property bool _irisOpen: root.roleOpen && root._sidebarShown

    // Drive the QML iris reveal from the combined visible state. Using only
    // `_sidebarShown` keeps the mask at 1 while the sidebar is closing (roleOpen
    // goes false first), so the panel would stay on screen.
    on_IrisOpenChanged: root.syncIris(root._irisOpen)

    function syncIris(open: bool): void {
        if (!Appearance.animationsEnabled || root.instantOpen) {
            root._irisProgress = open ? 1 : 0
            return
        }
        irisProgressAnim.stop()
        irisProgressAnim.duration = open
            ? (Appearance.animation?.elementMoveEnter?.duration ?? 400)
            : (Appearance.animation?.elementMoveExit?.duration ?? 200)
        irisProgressAnim.easing.bezierCurve
            = Appearance.animationCurves?.emphasizedDecel ?? [0.05, 0.7, 0.1, 1, 1, 1]
        irisProgressAnim.to = open ? 1 : 0
        irisProgressAnim.restart()
    }
    ...
    PanelWindow {
        ...
        Loader {
            ...
            // The surface stays mapped permanently, so niri can never replay its
            // layer open/close animation. Mask the content with the same
            // top-corner iris shader in QML, driven by the open/close pose.
            layer.enabled: root._irisActive
            layer.effect: IrisRevealEffect {
                progress: root._irisProgress
                // Each sidebar irises from the corner on its own edge: left from
                // top-left sweeping to bottom-right, right from top-right
                // sweeping to bottom-left.
                mirrored: !root.isLeftEdge
                w: sidebarContentLoader.width
                h: sidebarContentLoader.height
            }
