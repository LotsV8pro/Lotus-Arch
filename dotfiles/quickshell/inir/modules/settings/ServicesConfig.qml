        ...
        SettingsGroup {
            ...
            RowLayout {
                ...
                StyledText {
                    ...
                    text: Translation.tr("Location mode")
                    wrapMode: Text.Wrap
                }
                StyledComboBox {
                    Layout.preferredWidth: Math.min(210, Math.max(140, root.width * 0.38))
                    model: [
                        { displayName: Translation.tr("Auto (IP or GPS)"), value: "auto" },
                        { displayName: Translation.tr("Manual (city or coordinates)"), value: "manual" }
                    ]
                    textRole: "displayName"
                    currentIndex: Math.max(0,
                        (["auto", "manual"].indexOf(Config.options?.bar?.weather?.mode ?? "auto")))
                    onActivated: index => Config.setNestedValue("bar.weather.mode",
                        ["auto", "manual"][index])
                }
            }
            ...
            SettingsSwitch {
                ...
                enabled: (Config.options?.bar?.weather?.enable ?? false)
                         && (Config.options?.bar?.weather?.mode ?? "auto") === "auto"
                StyledToolTip {
                    text: Translation.tr("Uses GPS in auto mode before falling back to IP detection")
                }
            }
            ...
            ColumnLayout {
                ...
                spacing: 4
                enabled: Config.options?.bar?.weather?.enable ?? false
                opacity: (Config.options?.bar?.weather?.mode ?? "auto") === "manual" ? 1 : 0.5
            ...
            ColumnLayout {
                ...
                StyledText {
                    text: Translation.tr("Coordinates (optional, overrides city)")
                    color: Appearance.colors.colOnSurfaceVariant
                    font.pixelSize: Appearance.font.pixelSize.small
                }
