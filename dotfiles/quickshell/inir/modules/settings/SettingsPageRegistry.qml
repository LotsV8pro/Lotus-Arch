...
Singleton {
    ...
    readonly property var pages: [
        ...
        {
            key: "calendar-sync",
            name: Translation.tr("Calendar Sync"),
            icon: "event_repeat",
            desc: Translation.tr("Sync external calendars via ICS/iCal"),
            essential: false,
            component: "modules/settings/CalendarSyncGuide.qml"
        },
        {
            key: "waybar-editor",
            name: Translation.tr("Waybar Editor"),
            icon: "bar_chart",
    ...
    // Helper: should page (by its compositor field) be shown on this compositor?
    function compositorVisible(page: var): bool {
        if (!page || !page.compositor) return true
        const isNiri = (typeof CompositorService !== "undefined"
            && CompositorService.isNiri === true)
        if (page.compositor === "niri") return isNiri
        if (page.compositor === "hyprland") return !isNiri
        return true
    }
    ...
    function searchIndex(): var {
        ...
        // =====================================================================
        // Calendar Sync (page 28)
        // =====================================================================
        {
            pageIndex: 28, pageName: root.pages[28].name,
            section: Translation.tr("Calendar Sync"),
            label: Translation.tr("Calendar Sync"),
            description: Translation.tr("Sync external calendars through ICS/iCal URLs"),
            keywords: ["calendar", "sync", "ics", "ical", "caldav", "google", "feed", "fecha", "evento", "fuente", "source"]
        },
        {
            pageIndex: 28, pageName: root.pages[28].name,
            section: Translation.tr("Calendar Sync"),
            label: Translation.tr("Add a calendar source"),
            description: Translation.tr("Add your first ICS/iCal calendar source"),
            keywords: ["calendar", "source", "add", "ics", "ical", "google", "url", "enlace"]
        },

        // =====================================================================
        // Waybar Editor (page 29)
        // =====================================================================
        {
            pageIndex: 29, pageName: root.pages[29].name,
            section: Translation.tr("Waybar Editor"),
            label: Translation.tr("Waybar Editor"),
            description: Translation.tr("Switch waybar profiles, reload and open its config"),
            keywords: ["waybar", "bar", "barra", "perfil", "profile", "config", "configuración", "theme", "tema", "reload", "recargar"]
        }
        ]
        return _staticSearchIndex
    }
