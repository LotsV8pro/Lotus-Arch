...
Scope {
    ...
    Loader {
        ...
        sourceComponent: PanelWindow {
            ...
            // Stay visible during the close-animation window so the scrim
            // fade-out renders; the Loader tears down after closeAnimTimer
            // fires. The surface is never recreated mid-close.
...
// ── Floating settings card ──
            Rectangle {
                ...
                // No scale/opacity fade — the card follows the panel lifecycle so
                // the whole surface tears down atomically with closeAnimTimer,
                // avoiding the opacity cut that flashed on close.
                visible: root.settingsOpen || root._closeAnimRunning
    ...
    readonly property var visibleNavItems: {
        ...
        for (var c = 0; c < navCategories.length; c++) {
            ...
            for (var p = 0; p < cat.pages.length; p++) {
                ...
                if (!SettingsPageRegistry.compositorVisible(overlayPages[pageIdx])) continue;
