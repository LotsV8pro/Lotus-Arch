...
Item {
    ...
    // ─── Color hue filter (skwd: dynamic hue dots + tone drill-down) ───
    // -1=no filter, 0-9=named color groups (9=Brown), 100=white, 99=black
    property int colorFilter: -1
    // Optional secondary tone filters applied on top of colorFilter:
    // brightnessTone: "" | "light" | "mid" | "dark"
    // saturationTone: "" | "vivid" | "muted"
    property string brightnessTone: ""
    property string saturationTone: ""
    // Smallest share (fraction of all pixels) a color family needs to be a
    // "predominant" color (STRICT). Below this the image is neutral (hue 99).
    // Matches DOM_SHARE in scripts/wallpaper-color.py and is baked into the
    // cache at generation time, so it is not read at runtime.
    readonly property real _minDominantShare: 0.30
    // A dot is shown for every hue family that has at least one image in the
    // folder. The 12-hue wheel (+ achromatic) caps the total at 13, so this
    // can never clutter, and guarantees even a single rare wallpaper (e.g. a
    // lone magenta) still gets a dot representing it.
    readonly property real _minDotShare: 0.0
    // Persisted color cache, keyed by full path -> {hue, sat, lit, share}.
    property var _colorsDb: ({})
    property bool _colorsLoaded: false
    // Dynamic per-folder color dots, recomputed when the folder/cache changes.
    // Each entry: { index(0-9|99|100), bucket, count, share, color, isDark }
    property var _folderColorDots: []
    ...
    // Colors are keyed by the FULL wallpaper path (not filename): the user's
    // library has thousands of duplicate filenames across subfolders, so a
    // filename-only key made the color filter collide and show wrong results.
    function _getHueBucket(path: string): int {
        const entry = _colorsDb[path]
        return entry ? (entry.hue ?? -1) : -1
    }

    function _getSaturation(path: string): real {
        const entry = _colorsDb[path]
        return entry ? (entry.sat ?? 0) : 0
    }

    function _getLightness(path: string): real {
        const entry = _colorsDb[path]
        return entry ? (entry.lit ?? 0) : 0
    }

    function _getShare(path: string): real {
        const entry = _colorsDb[path]
        return entry ? (entry.share ?? 0) : 0
    }

    // True when this wallpaper has a known predominant color. Predominance is
    // already baked into the cache: hue 0-9 means that color group dominates
    // (9 = Brown), hue 99 = black, hue 100 = white. Only files
    // that haven't been analyzed yet (no entry / hue -1) are excluded.
    function _isPredominant(path: string): bool {
        const entry = _colorsDb[path]
        if (!entry) return false
        const hue = entry.hue ?? -1
        return hue >= 0 && hue <= 100
    }

    function _matchesTone(path: string): bool {
        if (brightnessTone === "" && saturationTone === "") return true
        const s = _getSaturation(path)
        const l = _getLightness(path)
        if (brightnessTone !== "") {
            const isLight = l >= 0.58
            const isDark = l <= 0.34
            if (brightnessTone === "light" && !isLight) return false
            if (brightnessTone === "dark" && !isDark) return false
            if (brightnessTone === "mid" && (isLight || isDark)) return false
        }
        if (saturationTone !== "") {
            // Named colors already need sat >= SAT_NAME (0.34) to count, so the
            // vivid/muted split sits higher to keep both halves meaningful.
            const vivid = s >= 0.45
            if (saturationTone === "vivid" && !vivid) return false
            if (saturationTone === "muted" && vivid) return false
        }
        return true
    }

    // Rebuild the dynamic per-folder color dot list from the images in the
    // current folder. Drops colors that only appear in a tiny number of images.
    function _rebuildColorDots(): void {
        if (!_colorsLoaded) { _folderColorDots = []; return }
        const buckets = {}
        let totalImages = 0
        for (let i = 0; i < totalCount; i++) {
            ...
            totalImages++
            const h = _getHueBucket(fpath)
            buckets[h] = (buckets[h] ?? 0) + 1
        }
        const dots = []
        // Display hue (degrees) for each of the 9 rainbow color groups 0..8.
        // Order: Red, Orange, Yellow, Green, Cyan, Blue, Violet, Purple, Pink.
        const DISPLAY_HUES = [0, 30, 60, 120, 180, 220, 265, 300, 330]
        for (const key of Object.keys(buckets)) {
            const b = parseInt(key, 10)
            const count = buckets[b]
            if (totalImages > 0 && (count / totalImages) < _minDotShare) continue
            const isDark = Appearance.m3colors.darkmode
            let color
            if (b === 99) {
                // Black: dark monochrome / dark wallpaper dot
                color = Qt.hsla(0, 0, isDark ? 0.05 : 0.08, 1.0)
            } else if (b === 100) {
                // White: light monochrome / light wallpaper dot
                color = Qt.hsla(0, 0, isDark ? 0.85 : 0.95, 1.0)
            } else if (b === 9) {
                // Brown: warm hue but muddy/earth-tone (tan, beige, gruvbox)
                const bh = isDark ? 0.16 : 0.20
                color = Qt.hsla(0.075, 0.45, bh, 1.0)
            } else if (b >= 0 && b < 9) {
                const hue = DISPLAY_HUES[b]
                color = Qt.hsla(hue / 360.0, isDark ? 0.55 : 0.60, isDark ? 0.60 : 0.55, 1.0)
            } else {
                color = Qt.hsla(isDark ? 0.55 : 0.50, 0.5, 0.5, 1.0)
            }
            dots.push({
                index: b,
                bucket: b,
        ...
        // order: achromatic (black) last, otherwise by color group
        dots.sort((a, b) => a.bucket === 99 ? 1 : (b.bucket === 99 ? -1 : a.bucket - b.bucket))
        _folderColorDots = dots
    }

    // (mtime sort uses model-index reversal, not per-file mtime lookup)

    onColorFilterChanged: {
        // Switching to a different color (or clearing the filter) resets any
        // tone drill-down so it never leaks across colors.
        if (brightnessTone !== "") brightnessTone = ""
        if (saturationTone !== "") saturationTone = ""
        _flippedImageIndex = -1
        _rebuildAndSync()
    }
    onBrightnessToneChanged: {
        if (colorFilter === -1) return
        _flippedImageIndex = -1
        _rebuildAndSync()
    }
    onSaturationToneChanged: {
        if (colorFilter === -1) return
        _flippedImageIndex = -1
        _rebuildAndSync()
    }
    ...
    function _rebuildIndexMaps(): void {
        ...
        for (let i = 0; i < totalCount; i++) {
            ...
            if (isDir) {
                ...
                if (typeFilter !== 0
                    ...
                    continue
                // Color filter: only wallpapers whose analyzed PREDOMINANT color
                // falls in the selected hue bucket (and any active tone filters)
                // are shown. Images are classified "predominant" by the share of
                // their dominant hue family; tiny color accents don't count. The
                // per-batch rebuild below adds any newly-analyzed items live.
                const fpath = FileUtils.trimFileProtocol(folderModel.get(i, "filePath") ?? "")
                if (colorFilter !== -1) {
                    const hue = _getHueBucket(fpath)
                    if (hue !== colorFilter) continue
                    if (!_isPredominant(fpath)) continue
                    if (!_matchesTone(fpath)) continue
                }
                // Favourites filter
        ...
        if (sortMode === "color" && _colorsLoaded) {
            imgMap.sort((a, b) => {
                const fpA = FileUtils.trimFileProtocol(folderModel.get(a, "filePath") ?? "")
                const fpB = FileUtils.trimFileProtocol(folderModel.get(b, "filePath") ?? "")
                const hueA = _getHueBucket(fpA)
                const hueB = _getHueBucket(fpB)
                // Achromatic (black 99, white 100) sorts last, white before black
                const sortA = hueA === 99 ? 101 : (hueA === 100 ? 100 : (hueA < 0 ? 102 : hueA))
                const sortB = hueB === 99 ? 101 : (hueB === 100 ? 100 : (hueB < 0 ? 102 : hueB))
                if (sortA !== sortB) return sortA - sortB
                return _getSaturation(fpB) - _getSaturation(fpA)
            })
        } else {
    ...
    function _runNextColorBatch(): void {
        ...
        // Delegate predominant-color analysis to the shared Python helper so the
        // live analyzer matches the pre-generated cache exactly (summed hue
        // family + a minimum dominant share; tiny color accents don't count).
        const paths = batch.map(b => b.path)
        _colorAnalysisProc.command = [
            "/usr/bin/python3",
            Directories.scriptsPath + "/wallpaper-color.py"
        ].concat(paths)
    ...
    Process {
        ...
        onExited: (exitCode, exitStatus) => {
            ...
            for (const line of _resultLines) {
                ...
                const pathKey = line.substring(0, tabIdx)
                const data = line.substring(tabIdx + 1).trim()
                if (data === "ERR" || data.length === 0) continue
                // Helper emits <path>\t{"hue":..,"sat":..,"lit":..,"share":..}
                let obj
                try { obj = JSON.parse(data) } catch (e) { continue }
                if (typeof obj.hue !== "number") continue
                db[pathKey] = {
                    hue: obj.hue,
                    sat: obj.sat ?? 0,
                    lit: obj.lit ?? 0,
            ...
            if (changed) {
                root._colorsDb = db
                // Newly-analyzed images may add/remove a color dot (and could
                // alter the filtered list), so refresh the folder dots too.
                root._rebuildColorDots()
            }
            // While a color filter is active, re-run the index map as each batch
            // lands so newly-analyzed images narrow the filtered list live. Only
            // _rebuildIndexMaps (no re-sync) so the user's current position is
            // not yanked back to the start.
            if (changed && root.colorFilter !== -1)
    ...
    Item {
        ...
        Row {
            ...
            // ─ Color dots (dynamic: only the colors actually found in this folder) ─
            Repeater {
                model: root._folderColorDots

                Rectangle {
                    id: colorDot
                    required property var modelData
                    readonly property int filterValue: modelData.bucket
            ...
            // ─ Tone drill-down chips (appear when a color is selected) ─
            Row {
                spacing: 4
                anchors.verticalCenter: parent.verticalCenter
                visible: root.colorFilter !== -1 && root._folderColorDots.length > 0

                // Divider before tone chips
                Rectangle {
                    width: 1; height: 14
                    anchors.verticalCenter: parent.verticalCenter
                    color: ColorUtils.applyAlpha(root.textColor, 0.18)
                ...
                Repeater {
                    model: [
                        { label: "Light",   value: "light", kind: "brightness" },
                        { label: "Mid",     value: "mid",   kind: "brightness" },
                        { label: "Dark",    value: "dark",  kind: "brightness" },
                        { label: "Vivid",   value: "vivid", kind: "saturation" },
                        { label: "Muted",   value: "muted", kind: "saturation" }
                    ]
                    ...
                    Rectangle {
                        ...
                        readonly property bool isSelected:
                            (modelData.kind === "brightness" && root.brightnessTone === modelData.value) ||
                            (modelData.kind === "saturation" && root.saturationTone === modelData.value)

                        anchors.verticalCenter: parent.verticalCenter
                        height: 22
                        radius: 11
                        color: isSelected ? Appearance.colors.colSecondaryContainer : "transparent"
                        ...
                        StyledText {
                            ...
                            text: toneChip.modelData.label
                            font.pixelSize: Appearance.font.pixelSize.smaller
                            font.weight: toneChip.isSelected ? Font.DemiBold : Font.Normal
                            color: toneChip.isSelected
                                ? Appearance.colors.colOnSecondaryContainer
                                : root.textColor
                            opacity: toneChip.isSelected ? 1.0 : 0.72
                        }
                        ...
                        MouseArea {
                            ...
                            cursorShape: Qt.PointingHandCursor
                            onClicked: {
                                const v = toneChip.modelData.value
                                const k = toneChip.modelData.kind
                                if (k === "brightness")
                                    root.brightnessTone = (root.brightnessTone === v) ? "" : v
                                else
                                    root.saturationTone = (root.saturationTone === v) ? "" : v
                            }
                        }
                    }
                }
