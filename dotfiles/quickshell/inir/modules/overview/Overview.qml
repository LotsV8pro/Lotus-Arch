    ...
    Variants {
        ...
        PanelWindow {
            ...
            Column {
                ...
                // Iris reveal mask (matches OSD).  In stage mode this clips the
                // column with an expanding circle driven by openProgress, matching the
                // overlay iris niri uses for menus.  Orbit mode keeps its own layer
                // for desaturation; the iris is handled per-scene inside.
                layer.enabled: columnLayout.visible
                layer.effect: IrisRevealEffect {
                    progress: columnLayout.openProgress
                    mirrored: false
                    w: columnLayout.width
                    h: columnLayout.height
                }
                ...
                // Opacity stays at 1; the IrisRevealEffect mask above drives the
                // close animation, so the slow decel tail collapses via the iris.
                ...
                // Iris reveal handles the close mask; keep opacity at 1 so the
                // effect is fully visible at every progress step.
                ...
                Loader {
                    ...
                    // Stay mounted through the whole close animation: root.visible
                    // only drops once _overviewCloseTimer fires, so the loader (and
                    // everything inside it) survives the fade-out instead of being
                    // torn down the instant shouldShow goes false.
                    active: root.visible
                ...
                Loader {
                    ...
                    visible: active && status === Loader.Ready
                        && root.visible && (root.searchingText == "")
