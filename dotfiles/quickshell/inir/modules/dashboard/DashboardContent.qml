...
Item {
    ...
    // Widgets that absorb the column's remaining height: the notepad (a writing
    // surface) and the to-do list (a long list benefits from the extra room).
    // Kept otherwise minimal so a column never shows two half-empty stretched cards.
    readonly property var _fillIds: ["notes", "todo"]
    ...
    Rectangle {
        ...
        Component { id: todoDetailComponent; TodoWidget { showExpandButton: false } }
        ...
        DashCard {
            ...
            z: 28
            forceOpaque: true
            // Match the dashboard's own panel tone, but neutralized: the dark
            // base read slightly rose-tinted, so the full view uses a pure-gray
            // equivalent at the same luminance — the dashboard feel without the
            // pink cast bleeding into the detail surface or its accents.
            surfaceColor: Appearance.zzzEverywhere ? "" : ColorUtils.grayscale(background.color)
            // Open/close animation mirrors niri's terminal window iris: the surface
            // reveals with a circular iris sweeping out from the top-left corner
            // and folds back into it when collapsing.
            property real _irisProgress: 0
            readonly property bool _irisActive: root.detailOpen || detailSurface._irisProgress > 0
            visible: detailSurface._irisActive
            layer.enabled: detailSurface._irisActive
            layer.effect: IrisRevealEffect {
                progress: detailSurface._irisProgress
                mirrored: false
                w: detailSurface.width
                h: detailSurface.height
            }
            ...
            states: [
                State {
                    name: "open"
                    when: root.detailOpen
                    PropertyChanges {
                        target: detailSurface
                        _irisProgress: 1
                    }
                },
                State {
            ...
            Loader {
                Layout.fillWidth: true
                Layout.fillHeight: true
                active: root.detailOpen || detailSurface._irisProgress > 0
