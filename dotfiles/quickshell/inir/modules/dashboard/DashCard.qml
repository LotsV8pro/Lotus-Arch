...
Rectangle {
    ...
    // Full-detail overlays (expanded widget views) must stay solid so the
    // dashboard behind them doesn't bleed through the reduced card opacity.
    property bool forceOpaque: false
    // Surface tone override: lets a full-detail overlay mimic the dashboard's
    // own panel background instead of reading as a distinct (lighter) card.
    ...
    color: {
        // surfaceColor is a Qt.rgba color (no .length) once set, so compare
        // against the "" default directly instead of probing length.
        const base = root.surfaceColor !== "" && root.surfaceColor != null
            ? root.surfaceColor
            : (zzzEverywhere ? Appearance.zzz.paper
                : Appearance.angelEverywhere ? Appearance.angel.colGlassCard
                : inirEverywhere ? Appearance.inir.colLayer1
                : auroraEverywhere ? Appearance.aurora.colSubSurface
                : Appearance.colors.colLayer1)
        if (root.forceOpaque) return ColorUtils.applyAlpha(base, 1)
