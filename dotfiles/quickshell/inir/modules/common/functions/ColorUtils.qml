...
Singleton {
    ...
    /**
     * Returns a pure-gray version of a color. Same perceived luminance as the
     * input but with zero hue/saturation — used to strip the warm/rose cast
     * out of translucent surfaces without changing how dark they read.
     *
     * @param {string} color - The base color (any Qt.color-compatible string).
     * @param {number} alpha - Desired alpha (0-1). Defaults to 1 (opaque).
     * @returns {Qt.rgba} A neutral grayscale color.
     */
    function grayscale(color, alpha = 1) {
        var c = Qt.color(color);
        if (!c.valid) return Qt.rgba(0, 0, 0, Math.max(0, Math.min(1, alpha)));
        var g = 0.299 * c.r + 0.587 * c.g + 0.114 * c.b;
        return Qt.rgba(g, g, g, alpha);
    }
