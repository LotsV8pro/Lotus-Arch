...
Item {
    ...
    Loader {
        ...
        active: root.popupMode === "bar"

        sourceComponent: PopupWindow {
            id: barMediaPopup
            visible: root.barMediaPopupVisible
            color: "transparent"
            anchor {
            ...
            Component.onCompleted: barMediaPopup.visible = root.barMediaPopupVisible

            Connections {
                target: root
                function onBarMediaPopupVisibleChanged() {
                    if (root.barMediaPopupVisible) {
                        barMediaCloseTimer.stop()
                        barMediaPopup.visible = true
                    } else {
                        barMediaCloseTimer.restart()
                    }
                }
            }

            Timer {
                id: barMediaCloseTimer
                // Keep the popup alive through the complete exit transition.
                interval: Math.max(50, Appearance.animation.elementMoveFast.duration + 40)
                onTriggered: barMediaPopup.visible = false
            }
