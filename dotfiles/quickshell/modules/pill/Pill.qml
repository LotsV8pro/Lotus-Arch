...
Item {
    ...
    readonly property point soulPoint: {
        ...
        if (soulTarget === "systemUpdate")
            return sysUpdateIcon.mapToItem(pill, sysUpdateIcon.width / 2, sysUpdateIcon.height + drop * 0.55);
        if (soulTarget === "mixer")
    ...
    Item {
        ...
        Row {
            ...
            Rectangle {
                ...
                height: 14 * pill.s
                radius: 7 * pill.s
                color: Updates.updateStronglyAdvised ? PillTheme.vermLit : PillTheme.subtle
                ...
                GlyphIcon {
                    ...
                    height: 9 * pill.s
                    name: "download"
                    color: Updates.updateStronglyAdvised ? PillTheme.cream : PillTheme.cardBg
                    stroke: 2
                }
            }
        }
        ...
        Row {
            ...
            Row {
                ...
                Item {
                    id: sysUpdateIcon
                    anchors.verticalCenter: parent.verticalCenter
                    visible: (pill.hoverModules?.systemUpdate ?? true) && Updates.available && Updates.count > 0
                    width: pill.iconPx
                    height: pill.iconPx
                    ...
                    GlyphIcon {
                        anchors.fill: parent
                        name: "download"
                        color: sysUpdateArea.containsMouse ? PillTheme.cream : (Updates.updateStronglyAdvised ? PillTheme.vermLit : PillTheme.iconDim)
                        stroke: 1.7
                    }
                    ...
                    Rectangle {
                        ...
                        anchors.topMargin: -2 * pill.s
                        anchors.rightMargin: -2 * pill.s
                        width: Math.max(10 * pill.s, sysUpdateBadge.implicitWidth + 4 * pill.s)
                        height: 10 * pill.s
                        radius: 5 * pill.s
                        color: Updates.updateStronglyAdvised ? PillTheme.vermLit : PillTheme.subtle
                        Text {
                    ...
                    MouseArea {
                        ...
                        onContainsMouseChanged: if (containsMouse) pill.soulTarget = "systemUpdate"
                    }
                }

                // Hairline between the status cluster (weather/tray/wifi/battery/
                // inbox/sysUpdate) and the tool surfaces — the row reads as sections, not
                ...
                Rectangle {
                    ...
                    height: 18 * pill.s
                    color: PillTheme.hair
                    visible: (weatherGlance.visible || trayRowItem.visible || wifiIcon.visible || batteryIcon.visible || inboxIcon.visible || sysUpdateIcon.visible)
