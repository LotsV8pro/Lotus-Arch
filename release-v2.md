...
- Phase 6 deploy now **rewrites every hardcoded `/home/lots` path** to the real `$HOME` at install time — the repo no longer forces you to be user `lots`.
...
- `steam-shader-limit.service` → `%h`; scripts use `$HOME` instead of `/home/lots`.
